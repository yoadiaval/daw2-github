...
export class NominasService {
  ...
  guardaNuevaNomina(nomina: Nomina) {
    ...
    this.nominas.push(
      new Nomina(ultimoId + 1, nomina.fecha, nomina.bruto, nomina.retencion)
    );
  }
  ...
  obtengoNomina(nnomina: number): Nomina {
    let indice = this.nominas.findIndex((item) => item.numero == nnomina);
    let tmpNomina = new Nomina(
      this.nominas[indice].numero,
      this.nominas[indice].fecha,
      this.nominas[indice].bruto,
      this.nominas[indice].retencion
    );
    return tmpNomina;
  }
}
