  ...
  guardaNuevaFactura( factura:Factura){
   ...
   factura.iva, factura.cantidad));
    ...
    modificaFactura(nfactura:number, factura:Factura){
      let indice = this.facturas.findIndex(item => item.numero == nfactura);/*encuentra en las facturas el índice de la factura que necesito*/
      ...
      obtengoFactura(nfactura:number):Factura{
        let indice = this.facturas.findIndex((item) => item.numero == nfactura);
        let tmpFactura = new Factura(
          this.facturas[indice].numero,
          this.facturas[indice].fecha,
          this.facturas[indice].iva,
          this.facturas[indice].cantidad
        );
        return tmpFactura;
      } 
