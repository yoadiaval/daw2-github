...
function accionVolver(event) {
  var b = document.getElementById("ligthbox");
  b.classList.add("lb-oculto"); /*Se oculta el modal*/
/*Pausa el video si se estuviera ejecutando*/
  if (medio.play) {
    medio.pause();
  }

  body.classList.remove("body-lb-mode");
}
...
function mostrarLB() {
  ...
  body.classList.add(
    "body-lb-mode"
  ); /*Agrega estilos especiales a la clase body para que no se vea la barra de scroll*/
...
function iniciar() {
  ...
  /*carga de elementos light-box y asignacion de eventos*/
  volver = document.getElementById("btn-volver");
  lb_opcion = document.getElementById("view-lb");
